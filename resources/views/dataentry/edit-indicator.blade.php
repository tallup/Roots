...
<head>
    ...
    <title>Edit Indicator - Data Entry - ROOTS</title>
...
<body>
    ...
    <div class="main-content">
        <div class="container">
            ...
            <div class="form-card">
                <h2 class="form-title">
                    <i class="fa fa-edit me-2"></i>Edit Indicator Performance Monitoring
                </h2>

                <form method="POST" action="{{ route('dataentry.edit-indicator.update', $indicator->indicator_id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="year" name="year" required>
                                ...
                                @foreach($years as $year)
                                    <option value="{{ $year }}" {{ $indicator->year == $year ? 'selected' : '' }}>{{ $year }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="quarter" name="quarter" required>
                                ...
                                @foreach($quarters as $quarter)
                                    <option value="{{ $quarter }}" {{ $indicator->proId == $quarter ? 'selected' : '' }}>{{ $quarter }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="indicator_type" name="indicator_type" required>
                                ...
                                @foreach($indicatorTypes as $type)
                                    <option value="{{ $type->indicatorId }}" {{ $indicator->indicatorId == $type->indicatorId ? 'selected' : '' }}>
                                        {{ $type->indicator_type }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="category" name="category" required>
                                ...
                                @foreach($indicatorCategories as $category)
                                    <option value="{{ $category }}" {{ $indicator->icat == $category ? 'selected' : '' }}>{{ $category }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="measurement" name="measurement" required>
                                ...
                                @foreach($measurements as $measurement)
                                    <option value="{{ $measurement }}" {{ $indicator->measuId == $measurement ? 'selected' : '' }}>{{ $measurement }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control" id="baseline" name="baseline" 
                                   placeholder="Enter baseline value" value="{{ $indicator->baseline }}" required>
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="number" class="form-control" id="target" name="target" 
                                   placeholder="Enter target value" value="{{ $indicator->target }}" step="0.01" required>
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="number" class="form-control" id="achieved" name="achieved" 
                                   placeholder="Enter achieved value" value="{{ $indicator->acheived }}" step="0.01" required>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control" id="achievement_percentage" name="achievement_percentage" 
                                   placeholder="Calculated automatically" value="{{ $indicator->acheivement }}" step="0.01" readonly>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="frequency" name="frequency" required>
                                ...
                                @foreach($dataFrequencies as $frequency)
                                    <option value="{{ $frequency }}" {{ $indicator->data == $frequency ? 'selected' : '' }}>{{ $frequency }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            ...
                            <textarea class="form-control" id="indicator_breakdown_plan" name="indicator_breakdown_plan" rows="3" 
                                      placeholder="Enter" required>{{ $indicator->comment }}</textarea>
                        </div>
                        <div class="col-md-6">
                            ...
                            <textarea class="form-control" id="indicator_breakdown_achieved" name="indicator_breakdown_achieved" rows="3" 
                                      placeholder="Enter" required>{{ $indicator->commentAc }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-12">
                            ...
                            <textarea class="form-control" id="remarks" name="remarks" rows="3" 
                                      placeholder="Enter" required>{{ $indicator->rmk }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-12 text-center">
                            <button type="submit" class="btn btn-primary btn-lg" name="submit" id="submit">
                                <i class="fa fa-save me-2"></i>Update Indicator Data
                            </button>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Pre-populate indicator description if indicator type is selected
            var selectedType = $('#indicator_type').val();
            if (selectedType) {
                $('#indicator_type').trigger('change');
                
                // Set the correct description after options are loaded
                setTimeout(function() {
                    $('#indicator_description').val('{{ $indicator->indicator_desc }}');
                }, 100);
            }
        });
    </script>
