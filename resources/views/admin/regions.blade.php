...
<div class="main-content-fixed">
    <div class="regions-container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="page-title mb-0">Regions</h2>
            <div class="d-flex gap-2">
                <a href="{{ route('admin.regions-report') }}" class="btn btn-secondary">
                    <i class="fa fa-file-alt me-2"></i>View Report
                </a>
                <a href="{{ route('admin.add-region') }}" class="btn btn-primary">
                    <i class="fa fa-plus me-2"></i>Add New Region
                </a>
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table id="regionsTable" class="table table-striped table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $region->region_initial }}</td>
                        <td>{{ $region->region_name }}</td>
                        <td>
                            <div class="d-flex gap-1">
                                <a href="{{ route('admin.edit-region', $region->regId) }}" 
                                   class="btn btn-sm btn-edit">
                                    <i class="fa fa-edit"></i> Edit
                                </a>
                                <button type="button" class="btn btn-danger btn-sm delete-region" 
                                        data-id="{{ $region->regId }}" 
...
$(document).on('click', '.delete-region', function() {
    ...
    if (confirm('Are you sure you want to delete the region "' + name + '"?')) {
        ...
        form.method = 'POST';
        form.action = '{{ url("/admin/delete-region") }}/' + id;
