...
<head>
    ...
    <title>Payment Modes - ROOTS</title>
...
<div class="main-content-fixed">
    <div class="payment-modes-container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="page-title mb-0">Payment Modes</h2>
            <div class="d-flex gap-2">
                <a href="{{ route('admin.payment-modes-report') }}" class="btn btn-secondary">
                    <i class="fa fa-file-alt me-2"></i>View Report
                </a>
                <a href="{{ route('admin.add-payment-mode') }}" class="btn btn-primary">
                    <i class="fa fa-plus me-2"></i>Add New Payment Mode
                </a>
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table id="paymentModesTable" class="table table-striped table-hover">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Payment Mode</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $paymentMode->pay_mode }}</td>
                        <td>
                            <div class="d-flex gap-1">
                                <a href="{{ route('admin.edit-payment-mode', $paymentMode->payId) }}" 
                                   class="btn btn-sm btn-edit">
                                    <i class="fa fa-edit"></i> Edit
                                </a>
                                <button type="button" class="btn btn-danger btn-sm delete-payment-mode" 
                                        data-id="{{ $paymentMode->payId }}" 
                                        data-name="{{ $paymentMode->pay_mode }}">
...
function deletePaymentMode(id, name) {
    if (confirm('Are you sure you want to delete the payment mode "' + name + '"?')) {
        ...
        form.method = 'POST';
        form.action = '{{ url("/admin/delete-payment-mode") }}/' + id;
